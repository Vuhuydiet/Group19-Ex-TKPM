...
router.post('/',
    ...
    body('program').isObject(),
    body('program.id').isString().isLength({ min: 1, max: 255 }),
    body('program.name').isString().isLength({ min: 1, max: 255 }),
    body('program.description').isString().isLength({ min: 1, max: 255 }),
