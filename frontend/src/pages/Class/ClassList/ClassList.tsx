...
function ClassList() {
    ...
    useEffect(() => {
        ...
        if (search.trim() !== "") {
            ...
            processedClasses = processedClasses.filter(cls =>
                regex.test(cls.id) || regex.test(cls.courseId) || (cls.professorName && regex.test(cls.professorName))
            );
        }

        // 2. Sắp xếp (sort)
        if (sortBy === "ID") {
            // Sắp xếp theo ID lớp học
            processedClasses.sort((a, b) => a.id.localeCompare(b.id));
        } else if (sortBy === "Name") {
            // Vì không có 'Name' cho lớp học, ta có thể sắp xếp theo ID môn học như một lựa chọn hợp lý
            processedClasses.sort((a, b) => a.courseId.localeCompare(b.courseId));
        }

        setCloneClasses(processedClasses);
        setPage(1); // Reset về trang 1 mỗi khi lọc/sắp xếp

    }, [classes, search, sortBy]); // useEffect sẽ chạy lại mỗi khi các giá trị này thay đổi
                    ...
                    <div className="board__feature__sortfilter">
                        <div className="board__feature__item">
                            ...
                            >
                                ...
                                <option value="ID">ID</option>
                                <option value="Name">{t('tableHeading.courseId')}</option> {/* Thay "Name" bằng "Course ID" cho rõ nghĩa */}
                                <option value="">None</option>
                            </select>
                        </div>
                    </div>
